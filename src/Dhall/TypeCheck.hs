...
typeWithA tpa = loop
  where
    ...
    loop ctx e@(ExtendTypes l r) = do
        ...
        return (Const Type)
    loop ctx e@(ExtendRecordTypes l r) = do
        tL <- loop ctx l
        let l' = Dhall.Core.normalize l
        case tL of
            Const Type  -> return ()
            _           -> Left (TypeError ctx e (ExtendRecordTypesRequiresRecordType l l'))
        tR <- loop ctx r
        let r' = Dhall.Core.normalize r
        case tR of
            Const Type  -> return ()
            _           -> Left (TypeError ctx e (ExtendRecordTypesRequiresRecordType r r'))
...
data TypeMessage s a
    ...
    | ExtendRecordTypesRequiresRecordType (Expr s a) (Expr s a)
...
prettyTypeMessage (ExtendRecordTypesRequiresRecordType expr0 expr1) =
    ErrorMessages {..}
  where
    short = "❰///❱ requires arguments that are record types"
    ...
    long = 
        ...
        \" <> txt1 <> "\n\
        \                                                                                \n\
        \... which is not an record type literal                                         \n"
      where
        txt0 = insert expr0
        txt1 = insert expr1
